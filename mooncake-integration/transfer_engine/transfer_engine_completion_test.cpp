...
int main() {
    bool ok = true;
    ok &=
        require(static_cast<int>(BatchTransferCompletionStatus::COMPLETED) == 0,
    ...
    auto ticket = std::make_shared<BatchTransferTicket>(
        41, owner, [backend_status] { return backend_status->load(); },
    ...
    ok &= require(
        ticket->poll() == BatchTransferCompletionStatus::COMPLETION_UNKNOWN,
    ...
    ok &= require(!shouldRetryBatchTransfer(
                      BatchTransferCompletionStatus::COMPLETION_UNKNOWN),
                  "completion-unknown must never be retried");
    ok &= require(
        shouldRetryBatchTransfer(BatchTransferCompletionStatus::FAILED_DRAINED),
    ...
    auto pending_ticket = std::make_shared<BatchTransferTicket>(
        42, pending_owner, [pending_status] { return pending_status->load(); },
        [] { return true; });
    ...
    ok &= require(
        registry.drain(20) == BatchTransferCompletionStatus::FAILED_DRAINED,
    ...
    ok &= require(
        sticky_registry.drain(0) == BatchTransferCompletionStatus::COMPLETED,
    ...
    auto externally_failed_ticket = std::make_shared<BatchTransferTicket>(
        ...
        [externally_failed_status] { return externally_failed_status->load(); },
        [] { return true; });
    ...
    std::weak_ptr<OwnerProbe> weak_quarantined_resource = quarantined_resource;
    ...
    ok &=
        require(task_tracker.aggregate() == BatchTransferBackendStatus::FAILED,
