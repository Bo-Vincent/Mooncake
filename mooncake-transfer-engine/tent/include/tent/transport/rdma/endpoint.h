...
class RdmaEndPoint : public std::enable_shared_from_this<RdmaEndPoint> {
   ...
   public:
    ...
    TentRdmaCcRoute* bindCongestionRoute(TentRdmaCcRoute* route) {
        TentRdmaCcRoute* expected = nullptr;
        congestion_route_.compare_exchange_strong(expected, route,
                                                  std::memory_order_release,
                                                  std::memory_order_relaxed);
        return congestion_route_.load(std::memory_order_acquire);
    }
    TentRdmaCcRoute* congestionRoute() const {
        return congestion_route_.load(std::memory_order_acquire);
    }
#endif
   ...
   private:
    ...
    std::atomic<TentRdmaCcRoute*> congestion_route_{nullptr};
#endif
