...
`StoredResourceManifest` is the common persistent-resource base. Its
`StoredWeightManifest` specialization is the one immutable, address-free
description of a committed weight snapshot: resource identity, revision,
weight generation, tensor descriptors, logical fragment geometry, and Store
payload keys. GPU addresses, runtime instances, leases, and allocation owners
remain in live bindings and are checked while Store I/O is performed.
