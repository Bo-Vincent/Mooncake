...
class TransferRegistrationCleanupPendingError(TransferEngineError):
    """Registration cleanup is quarantined until retry reaches a terminal state."""
...
def _batch_transfer_with_completion_fence(
    ...
    *,
    ...
    if not callable(ticket_method):
        ...
        if result != 0:
            # The legacy synchronous API has no ticket. A non-zero result can
            # represent a timeout while native DMA still owns the allocation.
    ...
    def _retain_pending_registration_cleanup(
        ...
        *,
        ...
        if type(restart_required) is not bool:
            raise TypeError("restart_required must be a boolean")
        status = {
            TerminalTransferState.ABORTED: "ABORTED",
            TerminalTransferState.COMPLETED: "COMPLETED",
            TerminalTransferState.FAILED_DRAINED: "FAILED_DRAINED",
        }[terminal_state]
        ...
        for token in allocation_tokens:
            ...
            if token_id in token_ids:
                raise TransferEngineError(
                    "pending registration cleanup has duplicate allocation lifetime token"
                )
        ...
        with _pending_transfer_lock:
            _pending_transfers[pending_transfer_id] = _PendingTransfer(
                ...
                ticket=(
                    _UndrainableCompletionUnknownTicket()
                    if restart_required
                    else _TerminalCleanupTicket(status)
                ),
                registrations=set(registrations),
                resources=tuple(resources),
                allocation_tokens=tuple(allocation_tokens),
        ...
        try:
            ...
            for address in ordered_addresses:
                try:
                    ...
                    detail = (
                        "pending transfer unregister_memory outcome is unknown; "
                        f"engine restart required: {error}"
                    )
                    if isinstance(error, Exception):
                        raise TransferEngineError(detail) from error
            ...
            while True:
                with _pending_transfer_lock:
                    ...
                    if not current.allocation_tokens:
                        break
                    token = current.allocation_tokens[-1]
                try:
                    ...
                    with _pending_transfer_lock:
                        ...
                        if current is None:
                            raise TransferEngineError(
                                "pending transfer disappeared during allocation "
                                f"release: {pending_transfer_id}"
                            )
                        if (
                            not current.allocation_tokens
                            or current.allocation_tokens[-1] is not token
                        ):
                            raise TransferEngineError(
                                "pending transfer allocation release order changed"
                            )
                        current.allocation_tokens = current.allocation_tokens[:-1]
                except BaseException as error:
                    with _pending_transfer_lock:
                        current = _pending_transfers.get(pending_transfer_id)
                        if current is not None:
                            ...
                            current.restart_required = True
                    detail = (
                        "pending transfer allocation lifetime release outcome is "
                        f"unknown; engine restart required: {error}"
                    )
                    if isinstance(error, Exception):
                        raise TransferEngineError(detail) from error
                    add_note = getattr(error, "add_note", None)
