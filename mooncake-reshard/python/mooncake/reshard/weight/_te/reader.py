...
from .execution import (
    executor_requirements_for_operation_indices,
...
class MooncakeTransferEngineReader:
    ...
    def execute(
        ...
        *,
        ...
        if not target_executors:
            return ()
        local_operation_indices = operation_indices_for_executors(
            plan, target_executors, "target"
        )
        source_requirements = executor_requirements_for_operation_indices(
            plan, local_operation_indices, "source"
        )
        ...
        scoped_source_bindings = tuple(
            ...
            if (binding.instance_id, binding.participant_id) in source_requirements
        )
        if {
            (binding.instance_id, binding.participant_id)
            for binding in scoped_source_bindings
        } != set(source_requirements):
        ...
        with self.transfer_executor.submission() as submission:
            ...
            try:
                acquired_sources, source_tokens = acquire_weight_lifetime_tokens(
                    ...
                    plan=plan,
                    bindings=scoped_source_bindings,
                    side="source",
                    providers=source_allocation_guards,
                    rollback_finalizer=(
                        self.transfer_executor.finalize_terminal_resource_sets
                    ),
                    required_fragment_ids_by_key=source_requirements,
                )
                acquired_targets, target_tokens = acquire_weight_lifetime_tokens(
                    ...
                    required_fragment_ids_by_key=target_requirements,
                )
                ...
                if submission.physical_io_started:
                    terminal_state = TerminalTransferState.FAILED_DRAINED
                raise
            finally:
                token_sets = (
                    ...
                    if lifetime_tokens is not None
                        ...
                        token_set
                        for token_set in (target_tokens, source_tokens)
    ...
    def _execute_reserved(
        ...
        *,
        ...
        target_registrations: Optional[Sequence[MemoryRegistrationLease]] = None,
        lifetime_tokens: Optional[AllocationTokenSet] = None,
        target_required_fragment_ids: Sequence[RuntimeFragmentId],
    ) -> tuple[DirectReadReceipt, ...]:
        ...
        target_executors = validate_scoped_executor_snapshot(
            plan,
            target_placement,
            target_binding,
            "target",
            target_required_fragment_ids,
        )
        target_executors = select_worker_executors(
            target_executors, target_worker_id, "target"
        )
        ...
        expected_source_executors = executors_for_operation_indices(
            plan, local_operation_indices, "source"
        )
        source_requirements = executor_requirements_for_operation_indices(
            plan, local_operation_indices, "source"
        ...
        expected_source_keys = {
            ...
            for executor in expected_source_executors
        }
        source_pairs = pair_manifests(source_placement, source_bindings, "source")
        for placement, binding in source_pairs:
            binding_key = (binding.instance_id, binding.participant_id)
            if binding_key not in expected_source_keys:
                continue
            required_fragment_ids = source_requirements[binding_key]
            executors = validate_scoped_executor_snapshot(
                plan,
                placement,
                binding,
                "source",
            ...
            for fragment in binding.fragments:
                if fragment.fragment_id not in required_fragment_ids:
                    continue
        ...
        expected_source_executor_keys = {
            (executor.rank, executor.worker_id)
            for executor in expected_source_executors
        }
        ...
        for index in local_operation_indices:
    ...
    def pending_transfer_status(self, pending_transfer_id: str) -> str:
        return self.transfer_executor.pending_transfer_status(pending_transfer_id)
    ...
    def drain_pending_transfer(
        ...
        *,
        ...
        return self.transfer_executor.drain_pending_transfer(
            pending_transfer_id,
