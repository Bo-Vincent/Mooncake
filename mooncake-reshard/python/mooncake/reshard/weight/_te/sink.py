...
class MooncakeTransferEngineSink:
    ...
    def execute(
        ...
        *,
        ...
        scoped_target_bindings = tuple(
            ...
            if (binding.instance_id, binding.participant_id) in target_requirements
        )
        if {
            (binding.instance_id, binding.participant_id)
            for binding in scoped_target_bindings
        } != set(target_requirements):
        ...
        with self.transfer_executor.submission() as submission:
            ...
            try:
                ...
                acquired_targets, target_tokens = acquire_weight_lifetime_tokens(
                    transfer_id=transfer_id,
                    plan=plan,
                    bindings=scoped_target_bindings,
                    side="target",
    ...
    def _execute_reserved(
        ...
        *,
        ...
        source_registrations: Optional[Sequence[MemoryRegistrationLease]] = None,
        lifetime_tokens: Optional[AllocationTokenSet] = None,
        source_required_fragment_ids: Sequence[RuntimeFragmentId],
    ) -> tuple[DirectTransferReceipt, ...]:
        source_executors = validate_scoped_executor_snapshot(
            plan,
            source_placement,
            source_binding,
            "source",
            source_required_fragment_ids,
        )
        source_executors = select_worker_executors(
            source_executors, source_worker_id, "source"
        )
        ...
        local_operation_indices = operation_indices_for_executors(
            plan, source_executors, "source"
        )
        expected_target_executors = executors_for_operation_indices(
            plan, local_operation_indices, "target"
        )
        target_requirements = executor_requirements_for_operation_indices(
            plan, local_operation_indices, "target"
        ...
        expected_target_keys = {
            ...
            for executor in expected_target_executors
        }
        ...
        for placement, binding in target_pairs:
            ...
            if binding_key not in expected_target_keys:
                continue
            required_fragment_ids = target_requirements[binding_key]
