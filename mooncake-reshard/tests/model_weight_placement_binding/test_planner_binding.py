...
class PlacementPlannerTest(unittest.TestCase):
    def test_stored_source_requires_an_authoritative_manifest_at_bind(self) -> None:
        ...
        source = WeightManifest(
            ...
            revision="revision",
            weight_generation=1,
            group_id="weights/default/model/revision/1",
            manifest_key="weights/default/model/revision/1/manifest",
            tensors=(tensor,),
            fragments=(
                StoredFragment(
                    ...
                    global_offset=(0,),
                    local_shape=(8,),
                    object_key="weights/default/model/revision/1/payload/full",
                    object_offset=0,
                    nbytes=8,
                ),
            ),
        ...
        with self.assertRaisesRegex(
            ValueError, "stored source plan requires a source manifest"
        ):
            bind_logical_transfer_plan(logical, (split_target_binding(right_address=0xA000),))

        foreign = WeightManifest(
            namespace="default",
            resource_id="model",
            revision="revision",
            weight_generation=1,
            group_id="weights/default/other/revision/1",
            manifest_key="weights/default/other/revision/1/manifest",
            tensors=(tensor,),
            fragments=(
                StoredFragment(
                    fragment_id="stored-full",
                    ...
                    global_offset=(0,),
                    local_shape=(8,),
                    object_key="weights/default/other/revision/1/payload/full",
                    object_offset=0,
                    nbytes=8,
                ),
            ),
        ...
        with self.assertRaisesRegex(ValueError, "source manifest identity differs"):
            bind_logical_transfer_plan(
                logical,
                (split_target_binding(right_address=0xA000),),
                source_manifest=foreign,
            )
        ...
        with self.assertRaisesRegex(
            ValueError, "source manifest fragment snapshots differ"
        ):
            replace(
                logical,
