...
def allocation_guards_for_bindings(
    ...
    *,
    ...
    owner_binding_by_key = {
        (binding.instance_id, binding.participant_id): binding
        for binding in owner_bindings
    }
    if len(owner_binding_by_key) != len(owner_bindings):
        raise RuntimeError("CUDA allocation owner bindings contain duplicates")
    for binding in bindings:
        key = (binding.instance_id, binding.participant_id)
        owner_binding = owner_binding_by_key.get(key)
        if owner_binding is None or owner_binding != binding:
            raise RuntimeError("CUDA allocation owner binding differs")
    owners_by_key = {
