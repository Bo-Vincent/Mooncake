...
def multi_worker_participant_inputs(
    prefix: str,
    address_base: int,
) -> RuntimeInputs:
    rank = ParallelRank()
    tensors = tuple(
        TensorDescriptor(
            tensor_id=f"layers.0.{name}",
            global_shape=(4,),
            dtype="uint8",
            itemsize=1,
            shard_dims=(),
            layer_id=0,
            layout_fingerprint=f"sglang:qwen3.5:{name}:uint8:test",
            parallel_axes=(ReplicatedAxis("dp"),),
        )
    ...
    participant_id = placement.topology.participants[0].participant_id
    binding = WeightRuntimeBindingManifest(
        ...
        generation=1,
        lease_id=f"{prefix}-runtime-lease",
        fragments=tuple(
            runtime_fragment(
                placement_fragment_id=placement_fragment.placement_fragment_id,
                fragment_id=f"{prefix}-{index}-fragment",
                address=address_base + index * 0x1000,
                nbytes=placement_fragment.nbytes,
                worker_id=f"{prefix}-worker-{index}",
                endpoint=f"{prefix}-worker-{index}:12345",
                local_shape=placement_fragment.local_shape,
                itemsize=tensor.itemsize,
            )
            for index, (tensor, placement_fragment) in enumerate(
                zip(tensors, placement_fragments)
            )
        ),
    )
