...
def test_te_sink_requires_only_peers_used_by_the_local_source() -> None:
    sources = manifests(tp=2, prefix="source", address_base=0x10000)
    targets = manifests(tp=4, prefix="target", address_base=0x40000)
    relevant_targets = targets.bindings[:2]
    engine = FakeTransferEngine()
    ...
    receipts = MooncakeTransferEngineSink(engine).execute(
        ...
        target_registrations=registration_leases(
            RuntimeInputs(targets.placement, relevant_targets)
        ),
        source_allocation_guards=allocation_guards_for_bindings((sources.bindings[0],)),
        target_allocation_guards=allocation_guards_for_bindings(relevant_targets),
    )
...
def test_te_sink_scopes_peer_validation_within_one_participant() -> None:
...
@pytest.mark.parametrize("source_worker_id", ["", 1, "missing-worker"])
def test_te_sink_rejects_invalid_explicit_source_worker_selector(
    source_worker_id: object,
) -> None:
    sources = manifests(tp=1, prefix="source", address_base=0x10000)
    targets = manifests(tp=1, prefix="target", address_base=0x40000)

    with pytest.raises(TransferEngineError, match="source_worker_id|unknown source"):
...
def test_te_sink_executes_weight_ranges_through_scatter_ticket_api() -> None:
...
def test_te_sink_surfaces_endpoint_failure() -> None:
    ...
    def write_with_ticket(endpoint, *args):
        return (
            ticket
            if endpoint == "target-t1:12345"
            else FakeBatchTransferTicket(["COMPLETED"])
        )
