...
@pytest.mark.parametrize(
    ("ticket_status", "expected_terminal_state"),
    (
        ("COMPLETED", TerminalTransferState.COMPLETED),
        ("FAILED_DRAINED", TerminalTransferState.FAILED_DRAINED),
    ),
)
def test_te_unregister_failure_quarantines_until_retry_succeeds(
    executor_name: str,
    ticket_status: str,
    expected_terminal_state: TerminalTransferState,
) -> None:
    sources = manifests(tp=1, prefix="source", address_base=0x10000)
    targets = manifests(tp=1, prefix="target", address_base=0x40000)
    source_guards = allocation_guards(sources)
    target_guards = allocation_guards(targets)
    engine = FakeTransferEngine()
    unregister_results = [-7, -8, 0]

    def unregister_memory(address: int) -> int:
        engine.unregister_calls.append(address)
        return unregister_results.pop(0)

    engine.unregister_memory = unregister_memory
    if executor_name == "sink":
        executor = MooncakeTransferEngineSink(engine)
        ticket = FakeBatchTransferTicket([ticket_status])
        engine.batch_transfer_sync_write_with_ticket = lambda *args: ticket
        plan = plan_transfer(sources, targets)
        expected_address = 0x10000
    ...
    assert engine.unregister_calls == [expected_address]
    assert executor.pending_transfer_ids() == (pending_id,)
    assert executor.pending_transfer_status(pending_id) == ticket_status
    assert all(
        token.released_states == []
        for token in _guard_tokens(source_guards, target_guards)
    )
    ...
    assert executor.drain_pending_transfer(pending_id) == ticket_status
    ...
    assert all(
        token.released_states == [expected_terminal_state]
        for token in _guard_tokens(source_guards, target_guards)
    )
...
def test_te_pending_transfer_cannot_drain_before_resource_handoff(
    ...
    if executor_name == "sink":
        ...
        pending_owner = executor.transfer_executor
    ...
    original_retain = pending_owner.retain_pending_resources
    ...
    pending_owner.retain_pending_resources = block_resource_handoff
    execution_errors = []
