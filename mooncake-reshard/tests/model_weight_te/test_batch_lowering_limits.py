...
def test_te_adapters_reject_non_integer_limits(adapter, name, value) -> None:
    with pytest.raises(ValueError, match=name):
        adapter(FakeTransferEngine(), **{name: value})
...
def test_te_sink_lowers_multi_dim_regions_in_bounded_batches() -> None:
    sources = multi_dim_te_manifests("source", 0x10000, source=True)
    targets = multi_dim_te_manifests("target", 0x40000, source=False)
...
def test_te_reader_lowers_multi_dim_regions_in_bounded_batches() -> None:
    sources = multi_dim_te_manifests("source", 0x10000, source=True)
    target = participant_inputs(
        multi_dim_te_manifests("target", 0x40000, source=False), 1
    )
...
def test_te_rejects_multi_dim_region_above_lowering_limit(executor: str) -> None:
    sources = multi_dim_te_manifests("source", 0x10000, source=True)
    targets = multi_dim_te_manifests("target", 0x40000, source=False)
    engine = FakeTransferEngine()
