...
def test_upload_deduplicates_dp_and_commits_manifest_last() -> None:
    ...
    plan = weight_store.prepare_upload(
...
def test_store_manifest_json_round_trip_persists_weight_generation() -> None:
    ...
    decoded = WeightManifest.from_json(raw)
...
def test_prepare_upload_does_not_require_binding_for_empty_participant() -> None:
...
def test_same_revision_different_generations_have_disjoint_store_identity() -> None:
    ...
    plan_17 = weight_store.prepare_upload(
        generation_17.placement,
        generation_17.bindings,
    )
    plan_18 = weight_store.prepare_upload(
    ...
    manifest_17 = weight_store.commit(
        plan_17,
        upload_all(weight_store, plan_17, generation_17),
    )
    manifest_18 = weight_store.commit(
        plan_18,
...
def test_prepare_upload_matches_te_planner_dp_replica_selection() -> None:
    sources = source_manifests(dp=2, tp=2)
    targets = target_manifests(dp=1, tp=1)
    _, weight_store = make_weight_store()

    te_plan = plan_transfer(sources, targets)
    store_plan = weight_store.prepare_upload(sources.placement, sources.bindings)
...
def test_prepare_upload_rejects_dense_tp_shards_across_ep_replicas() -> None:
    tensor = replace(
        tensor_descriptor(),
