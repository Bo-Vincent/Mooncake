...
def test_store_load_holds_target_pin_through_sync_ranged_read() -> None:
    ...
    sources = source_manifests(dp=1, tp=1)
    upload_plan = staging_store.prepare_upload(sources.placement, sources.bindings)
    manifest = staging_store.commit(
        upload_plan,
...
def test_typed_pre_registered_lease_retains_pin_until_explicit_close() -> None:
    ...
    lease = weight_store.register_weight_buffers(
        sources.binding,
        fragment_ids=tuple(
            operation.source_binding.fragment_id for operation in plan.operations
        ),
