...
def test_store_preserves_expert_boxes_and_loads_cross_dim(
    ...
    upload_plan = weight_store.prepare_upload(sources.placement, sources.bindings)
    ...
    for target in bound_fragments(targets):
        assert bytes(target.owner) == expected_nd_fragment(target.placement)
...
def test_load_batches_multiple_local_target_buffers_in_one_request() -> None:
    ...
    store, weight_store = make_weight_store()
    upload_plan = weight_store.prepare_upload(source.placement, source.bindings)
    manifest = weight_store.commit(
