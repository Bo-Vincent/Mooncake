...
from mooncake.reshard.lifetime import AllocationFence, TerminalTransferState
...
@dataclass
class _BenchmarkAllocationToken:
    _fence: AllocationFence
    ...
    def release_after_terminal(self, terminal_state: TerminalTransferState) -> None:
        ...
        if self._released:
            raise RuntimeError("benchmark allocation token was already released")
        self._owners = ()
        self._released = True


class _BenchmarkAllocationGuard:
    def __init__(
        self,
    ...
    def acquire(
        ...
        *,
        ...
        if expected_binding != self._binding:
            raise ValueError("benchmark allocation binding changed")
        fragments = {
            fragment.fragment_id: fragment for fragment in self._binding.fragments
        }
        owners = tuple(
            (
                fragments[fragment_id].owner
                if fragments[fragment_id].owner is not None
                else self._fallback_owner
            )
            for fragment_id in required_fragment_ids
            if fragment_id in fragments
        )
        if len(owners) != len(required_fragment_ids) or any(
            owner is None for owner in owners
        ):
            raise ValueError("benchmark allocation owner is unavailable")
        token_id = f"benchmark:{transfer_id}:{uuid4().hex}"
        return AcquiredWeightBinding(
            binding=self._binding,
            token=_BenchmarkAllocationToken(
                weight_allocation_fence(
                    self._binding,
...
def benchmark_allocation_guards(
    bindings: Sequence[WeightRuntimeBindingManifest],
    *,
    fallback_owner: object | None = None,
) -> WeightAllocationGuardProviders:
    """Create guard providers for allocations owned by the benchmark session."""

    result = {
        (binding.instance_id, binding.participant_id): _BenchmarkAllocationGuard(
            binding,
            fallback_owner,
        )
        for binding in bindings
    ...
    if len(result) != len(bindings):
        raise ValueError("benchmark runtime bindings contain duplicate participants")
    return result
...
def execute_update(
    *,
    ...
    target_lifetime_owner: object | None = None,
    clock: Callable[[], float] = time.perf_counter,
) -> UpdateSample:
    ...
    receipts = tuple(
        ...
        for receipt in sink.execute(
            ...
            source_allocation_guards=benchmark_allocation_guards(source_bindings),
            target_allocation_guards=benchmark_allocation_guards(
                target_bindings,
                fallback_owner=target_lifetime_owner,
            ),
        )
    )
