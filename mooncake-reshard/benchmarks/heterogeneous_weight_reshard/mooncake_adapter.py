...
def _target_batch_counts(
    ...
    for source_executor in plan.source_executors:
        operations_by_endpoint: dict[str, list[TransferRegion]] = {}
        for index in plan.operation_indices_for_executor(
            source_executor,
            "source",
        ):
            region = plan.operations[index]
            operations_by_endpoint.setdefault(region.target.endpoint, []).append(region)
        for endpoint in sorted(operations_by_endpoint):
            regions = sorted(
                operations_by_endpoint[endpoint],
                key=lambda region: (
                    region.target.address + region.target_offset,
                    region.source.address + region.source_offset,
                ),
            )
            batch_operations = 0
            batch_workers: set[str] = set()
            for region in regions:
                worker_id = region.target.worker_id
                remaining = region.repeat
                if batch_operations:
                    take = min(
                        max_batch_operations - batch_operations,
                        remaining,
                    )
                    batch_operations += take
                    batch_workers.add(worker_id)
                    remaining -= take
                    if batch_operations == max_batch_operations:
                        for touched_worker in batch_workers:
                            counts_by_worker[touched_worker] += 1
                        batch_operations = 0
                        batch_workers = set()
                full_batches, remaining = divmod(remaining, max_batch_operations)
                counts_by_worker[worker_id] += full_batches
                if remaining:
                    batch_operations = remaining
                    batch_workers.add(worker_id)
            if batch_operations:
                for touched_worker in batch_workers:
                    counts_by_worker[touched_worker] += 1
...
def plan_mooncake_case(
    ...
    *,
    ...
    analysis_segment_limit = max(max_region_segments, prod(shape))
    logical_plan = plan_placement_transfer(
        source_topology.placement,
        target_topology.placement,
        planning_limits=PlanningLimits(
            max_segments_per_region=analysis_segment_limit,
            max_total_lowered_segments=max(
                10_000_000,
                analysis_segment_limit * target.total_ranks,
            ),
        ),
    )
