...
def _target_topology(case: BenchmarkCase) -> RuntimeTopology:
...
def _bound_plan(source: RuntimeTopology, target: RuntimeTopology):
...
def test_ready_message_round_trip_validates_registration_envelopes() -> None:
    case = _case()
    topology = _target_topology(case)
    envelopes = tuple(
        RegistrationEnvelope(
            allocation_id=f"allocation-{rank}",
            fragment_id=binding.fragments[0].fragment_id,
            device_id=rank,
            base_address=binding.fragments[0].address,
            nbytes=binding.fragments[0].nbytes,
            session_id="session-1",
            generation=9,
        )
        for rank, binding in enumerate(topology.bindings)
    )
    ...
    assert ready.placement == topology.placement
    assert ready.bindings == topology.bindings
...
def test_ready_message_rejects_stale_generation_and_out_of_bounds_fragment() -> None:
    topology = _target_topology(_case())
    envelopes = tuple(
        RegistrationEnvelope(
            allocation_id=f"allocation-{rank}",
            fragment_id=binding.fragments[0].fragment_id,
            device_id=rank,
            base_address=(
                binding.fragments[0].address + 1
                if rank == 0
                else binding.fragments[0].address
            ),
            nbytes=binding.fragments[0].nbytes,
            session_id="session-1",
            generation=9,
...
def test_cross_dim_expected_segments_are_grouped_by_source_shard() -> None:
    ...
    fragment = _target_topology(case).placement.parts[2].fragments[0]
...
def test_same_dim_expected_segment_does_not_expand_rows_or_elements() -> None:
    ...
    fragment = _target_topology(case).placement.parts[1].fragments[0]
