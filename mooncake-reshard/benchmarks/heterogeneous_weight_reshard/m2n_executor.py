...
_TIME_RE = re.compile(
    rf"^Time per iteration \(ms\):\s+Min=({_FLOAT_TOKEN})\s+" rf"Max=({_FLOAT_TOKEN})$",
    re.IGNORECASE,
)
...
_SHARD_HEADER_RE = re.compile(
    r"^Source shard dim: (\d+), Dest shard dim: (\d+) " r"\((same-dim|CROSS-DIM!)\)$",
    re.MULTILINE,
)
_ROLE_RE_TEMPLATE = (
...
_SUMMARY_SHARD_RE = re.compile(
    r"^Sharding: src_dim=(\d+), dst_dim=(\d+) " r"\((same-dim|cross-dim)\)$",
    re.MULTILINE,
)
