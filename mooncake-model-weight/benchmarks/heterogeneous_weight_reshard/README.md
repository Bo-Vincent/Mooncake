...
这个目录用同一份 logical tensor case 描述 Mooncake runtime manifest +
...
- 用非零 fake address 构造 Mooncake runtime manifest，并调用现有 N-D planner
  生成 `WeightTransferPlan` 的静态摘要。
...
- Mooncake source/target role 各自分配并注册本机 CUDA shard；target 只通过有界
  控制协议发布真实 runtime manifest 和 registration envelope。
...
python3 -m \
  ...
  --config mooncake-model-weight/benchmarks/heterogeneous_weight_reshard/cases.json \
...
计时复用 allocation、registration、manifest 和 plan；计时后 target 先清零，再执行
...
构建和裸传输，
不表达 runtime manifest 或异构 reshard，不能作为这组对比的 Mooncake 主结果。
...
- runtime manifest 是 Mooncake 物理地址、shape、dtype、layer/expert 语义的唯一
  事实来源。
