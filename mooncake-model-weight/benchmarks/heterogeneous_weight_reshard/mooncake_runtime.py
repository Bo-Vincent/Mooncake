        ...
        *,
        ...
        try:
            ...
            registrations = tuple(
                ...
                for index, (manifest, buffer) in enumerate(
                    zip(manifests, buffers, strict=True)
                )
            )
            return cls(
                case=case,
...
def verify_target_allocation(
    ...
    *,
    ...
    started = clock()
    checked_bytes = 0
    for manifest, buffer in zip(allocation.manifests, allocation.buffers, strict=True):
    ...
    return ValidationResult(
        ...
        fragment_count=len(allocation.manifests),
...
def run_source_session(
    ...
    *,
    ...
    plan = plan_runtime_transfer(
        source_allocation.manifests,
        target_ready.manifests,
    )
    plan_seconds = clock() - plan_started
    selected_replicas = {operation.source.rank.dp for operation in plan.operations}
    ...
    source_leases = registration_leases(source_allocation.manifests)
    target_leases = registration_leases(target_ready.manifests)

    first = execute_update(
        sink=sink,
        timed_engine=timed_engine,
        plan=plan,
        source_manifests=source_allocation.manifests,
        target_manifests=target_ready.manifests,
    ...
    return {
        "schema_version": 1,
        "backend": "mooncake-te-runtime-manifest",
