...
from .wire_protocol import runtime_manifest_from_wire, runtime_manifest_to_wire
...
def ready_message(
    *,
    ...
    return {
        ...
        "generation": generation,
        "endpoint": endpoint,
        "manifests": [runtime_manifest_to_wire(item) for item in manifests],
...
def parse_ready_message(
    ...
    *,
    ...
    manifests = tuple(runtime_manifest_from_wire(item) for item in raw["manifests"])
    registrations = tuple(
    ...
    if not manifests:
        raise ValueError("ready message must contain target manifests")
    ...
    if len(fragments) != sum(len(manifest.fragments) for manifest in manifests):
    ...
    for fragment_id, fragment in fragments.items():
        ...
        if fragment.lease_generation != expected_generation:
            raise ValueError("fragment generation mismatch")
