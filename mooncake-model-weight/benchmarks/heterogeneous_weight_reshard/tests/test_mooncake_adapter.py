...
from mooncake.model_weight import TransferPlan, TransferRegion
...
def test_summarizes_static_nd_plan_exactly(
    ...
    assert result.summary.manifest_count == manifest_count
...
def test_uses_nonzero_non_overlapping_fake_fragment_addresses() -> None:
    ...
    manifests = (*result.source_manifests, *result.target_manifests)
    fragments = tuple(
        fragment for manifest in manifests for fragment in manifest.fragments
    )

    assert len(ranges) == result.summary.manifest_count
    assert len(manifests) == result.summary.manifest_count
    assert ranges == sorted(
        (fragment.address, fragment.address + fragment.nbytes) for fragment in fragments
    )
    assert result.tensor_id == "benchmark_tensor"
    assert all(fragment.tensor_id == result.tensor_id for fragment in fragments)
    assert all(fragment.lease_generation == 1 for fragment in fragments)
...
def test_dp_source_replicas_are_selected_without_logical_duplication() -> None:
    ...
    assert result.summary.selected_source_manifest_count == 4
    assert result.summary.deduplicated_source_manifest_count == 4
