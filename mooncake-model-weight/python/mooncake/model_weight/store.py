...
class UploadOperation:
    ...
    source_runtime_lease_id: str | None = None
...
class WeightStore:
    ...
    def prepare_upload(
        ...
        *,
        ...
        runtime_lease_by_fragment = {
            fragment.fragment_id: manifest.lease_id
        ...
        for source in sources:
            ...
            operations.append(
                UploadOperation(
                    source=source,
                    target=target,
                    source_runtime_lease_id=runtime_lease_by_fragment[
                        source.fragment_id
                    ],
                )
            )
    ...
    def upload(
        ...
        *,
        ...
        for operation in expected_operations:
            ...
            if runtime_manifest.lease_id != operation.source_runtime_lease_id:
                raise WeightStoreError(
                    f"stale source lease: {operation.source.fragment_id}"
                )
        ...
        with self._registered(sources, pre_registered=pre_registered):
            for begin in range(0, len(local_operations), self.max_ranges_per_request):
                batch = local_operations[begin : begin + self.max_ranges_per_request]
