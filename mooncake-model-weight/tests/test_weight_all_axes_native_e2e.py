...
def _bind_target_placements(
    targets: Sequence[RuntimeManifest],
) -> tuple[tuple[TargetPlacementManifest, ...], tuple[RuntimeManifest, ...]]:
    placements = []
    bound_targets = []
    for target in targets:
        placement_fragments = tuple(
            PlacementFragment(
                placement_fragment_id=(
                    ...
                    or f"placement:{fragment.fragment_id}"
                ),
        ...
        generations = {fragment.lease_generation for fragment in target.fragments}
        ...
        binding = RuntimeBindingManifest(
            ...
            lease_id=target.lease_id or f"lease:{target.instance_id}",
            fragments=tuple(
                RuntimeBindingFragment(
                    placement_fragment_id=(
                        fragment.placement_fragment_id
        ...
        bound_targets.append(bind_runtime_manifest(placement, binding))
    return tuple(placements), tuple(bound_targets)
            ...
            if command[0] == "allocate":
                ...
                receipts = MooncakeTransferEngineReader(engine).execute(
                    ...
                    target_registrations=tuple(
                        MemoryRegistrationLease.from_fragment(
                            fragment,
                            runtime_lease_id=target.lease_id,
                        )
                        for fragment in target.fragments
                    ),
                )
...
def _execute_reader_fixture(
    ...
    *,
    placement_first: bool = False,
) -> None:
    sources = (fixture.source,)
    total_bytes = 0
    for declared_target in fixture.targets:
        target = declared_target
        if placement_first:
            placements, bound_targets = _bind_target_placements((declared_target,))
            logical = plan_runtime_transfer_to_local_target_placement(
                sources, placements[0]
            )
            assert all(
            ...
            plan = bind_logical_transfer_plan(
                logical, (target,), source_bindings=sources
            )
        else:
    ...
    *,
    ...
    for declared_target in fixture.targets:
        ...
        if placement_first:
            ...
            target = bound_targets[0]
            plan = bind_logical_transfer_plan(
                logical, (target,), source_bindings=fixture.sources
            )
        else:
...
def _execute_cross_dim_sink_fixture(
    ...
    *,
    ...
    if placement_first:
        placements, targets = _bind_target_placements(fixture.targets)
        logical = plan_runtime_transfer_to_target_placements(
            fixture.sources, placements
        )
        assert all(
            not hasattr(operation.target, "address") for operation in logical.operations
        )
        plan = bind_logical_transfer_plan(
            logical, targets, source_bindings=fixture.sources
        )
    else:
        plan = plan_runtime_transfer(fixture.sources, targets)
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_all_axes_fixture_executes_one_composite_plan_without_native_runtime(
    ...
    with (
        ...
        _execute_te_fixture(engine, fixture, placement_first=placement_first)


@pytest.mark.parametrize("placement_first", (False, True))
def test_reader_fixture_pulls_packed_tp1_weights_into_tp2_targets(
    placement_first: bool,
) -> None:
    engine = HostTransferEngine()
    ...
    with (
        ...
        _execute_reader_fixture(
            fixture, execute_reader, placement_first=placement_first
        )


@pytest.mark.parametrize("placement_first", (False, True))
def test_reader_fixture_reshards_independent_experts_across_dimensions(
    placement_first: bool,
) -> None:
    engine = HostTransferEngine()
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_sink_fixture_reshards_independent_experts_across_dimensions(
    placement_first: bool,
) -> None:
    engine = HostTransferEngine()
    fixture = _build_cross_dim_reader_fixture(
    ...
    with (
        ...
        _execute_cross_dim_sink_fixture(
            fixture, engine, placement_first=placement_first
        )
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_gpu_store_moves_weights_across_dp_tp_pp_ep_together(
    placement_first: bool,
) -> None:
    ...
    with ExitStack() as stack:
        ...
        try:
            ...
            with _registered_store_buffers(store, all_buffers):
                ...
                if placement_first:
                    placements, targets = _bind_target_placements(fixture.targets)
                    logical = plan_stored_transfer_to_target_placements(
                        loaded, placements
                    )
                    assert all(
                        not hasattr(operation.target, "address")
                    ...
                    load_plan = WeightLoadPlan(
                        manifest=loaded,
                        transfer=bind_logical_transfer_plan(logical, targets),
                    )
                else:
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_gpu_store_reshards_independent_experts_across_dimensions(
    placement_first: bool,
) -> None:
    from mooncake.store import MooncakeDistributedStore
...
def test_te_tcp_moves_weights_across_dp_tp_pp_ep_together(
    ...
    with ExitStack() as stack:
        ...
        _execute_te_fixture(source_engine, fixture, placement_first=placement_first)
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_te_reader_tcp_pulls_packed_tp1_weights_into_tp2_targets(
    placement_first: bool,
) -> None:
    from mooncake.engine import TransferEngine
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_te_reader_cuda_pulls_packed_tp1_weights_into_tp2_targets(
    placement_first: bool,
) -> None:
    from mooncake.engine import TransferEngine
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_te_sink_cuda_reshards_independent_experts_across_dimensions(
    placement_first: bool,
) -> None:
    from mooncake.engine import TransferEngine
    ...
    with (
        ...
        with _registered_engine_buffers(source_engine, fixture.source_buffers):
            _execute_cross_dim_sink_fixture(
                fixture, source_engine, placement_first=placement_first
            )
...
@pytest.mark.parametrize("placement_first", (False, True))
def test_te_reader_cuda_reshards_independent_experts_across_dimensions(
    placement_first: bool,
) -> None:
    from mooncake.engine import TransferEngine
