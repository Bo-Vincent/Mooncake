...
import mooncake.model_weight.planner as planner_module
...
def test_runtime_plan_records_manifest_lease_id_in_executor_snapshot(
    side: str,
) -> None:
    sources = tuple(
        replace(manifest, lease_id=f"source-lease-{index}")
    ...
    targets = tuple(
        replace(manifest, lease_id=f"target-lease-{index}")
        for index, manifest in enumerate(
            tp_manifests(
                tp=2,
                pp_rank=0,
    ...
    plan = plan_runtime_transfer(sources, targets)
    manifests = sources if side == "source" else targets
    ...
    assert [executor.runtime_lease_id for executor in executors] == [
        manifest.lease_id for manifest in manifests
    ]


@pytest.mark.parametrize("side", ["source", "target"])
def test_executor_snapshot_rejects_manifest_lease_id_change(side: str) -> None:
    source = replace(
        tp_manifests(
            tp=1,
            pp_rank=0,
    ...
    current = replace(manifest, lease_id=f"rotated-{side}-lease")

    with pytest.raises(ValueError, match=f"{side} executor snapshot mismatch"):
        resolve_executor_plans(plan, current, side)


def test_one_shot_executor_snapshot_accepts_missing_lease_id() -> None:
    source = tp_manifests(
        tp=1,
        pp_rank=0,
        ep_rank=0,
    ...
    assert plan.source_executors[0].runtime_lease_id is None
    assert plan.target_executors[0].runtime_lease_id is None
    assert resolve_executor_plans(plan, source, "source")
    assert resolve_executor_plans(plan, target, "target")
...
def test_runtime_plan_rejects_undeclared_cross_tensor_physical_overlap() -> None:
    tensors = tuple(
        replace(
            descriptor(),
            tensor_id=tensor_id,
            ...
            layer_id=None,
            expert_id=None,
            layout_fingerprint=f"independent:{tensor_id}",
        )
        for tensor_id in ("a.weight", "b.weight")
    )
    ...
    with pytest.raises(ValueError, match="runtime manifest address ranges overlap"):
        manifest("source", 0x10000)
...
def test_runtime_plan_rejects_cross_dim_alias_generation_mismatch() -> None:
    with pytest.raises(ValueError, match="inconsistent lease generations"):
        cross_dim_alias_manifests(target_alias_generation=2)
...
def test_runtime_plan_rejects_partially_overlapping_target_physical_ranges() -> None:
    ...
    manifest("source", (0x10000, 0x11000), target=False)
    with pytest.raises(ValueError, match="runtime manifest address ranges overlap"):
        manifest("target", (0x20000, 0x20020), target=True)
