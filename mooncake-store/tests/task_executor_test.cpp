...
public:
    ...
    void Record(const std::string& operation, const std::string& key, ErrorCode result) {
...
protected:
    ...
    QueryResult CreateQueryResultWithReplicas(const std::vector<Replica::Descriptor>& replicas) {
        std::vector<Replica::Descriptor> replicas_copy = replicas;  // Make a copy to move
        std::chrono::steady_clock::time_point lease_timeout = 
            std::chrono::steady_clock::now() + std::chrono::seconds(60);  // 60 second lease
...
TEST_F(TaskExecutorTest, TaskExecutorSubmitTask) {
    ...
    executor.SubmitTask(task, client_id);
...
TEST_F(TaskExecutorTest, TaskExecutorSubmitMultipleTasks) {
    ...
    for (int i = 0; i < 5; ++i) {
        Task task;
        task.id = generate_uuid();
        task.type = (i % 2 == 0) ? TaskType::REPLICA_COPY : TaskType::REPLICA_MOVE;
        task.status = TaskStatus::PENDING;
    ...
    for (int i = 0; i < 3; ++i) {
        threads.emplace_back([&executor, &client_id, &submitted_count, i]() {
            for (int j = 0; j < 2; ++j) {
                ...
                task.status = TaskStatus::PENDING;
                ReplicaCopyPayload payload;
                payload.key = "test_key_" + std::to_string(i) + "_" + std::to_string(j);
                payload.targets = {"target1"};
                struct_json::to_json(payload, task.payload);
...
TEST_F(TaskExecutorTest, TaskExecutorInvalidTaskType) {
    ...
    Task task;
    task.id = generate_uuid();
    task.type = static_cast<TaskType>(999); // Invalid task type
    task.status = TaskStatus::PENDING;
...
TEST_F(TaskExecutorTest, MemoryReplicaDescriptorHandling) {
    ...
    EXPECT_EQ(mem_desc.buffer_descriptor.transport_endpoint_, "test_segment:12345");
}
...
TEST_F(TaskExecutorTest, SourceSegmentExtraction) {
    ...
    auto memory_replica = CreateMemoryReplicaDescriptor(1, "memory_seg:12345", 1024);
...
TEST_F(TaskExecutorTest, ExecuteTaskWithInvalidCopyPayload) {
    ...
    task.type = TaskType::REPLICA_COPY;
    task.status = TaskStatus::PENDING; 
    task.payload = R"({"key": "some_key", "targets": ["t1", "t2" })"; // 注意花括号不匹配

    executor.SubmitTask(task, client_id);

    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    SUCCEED() << "Executor handled invalid JSON payload gracefully without crashing.";
...
TEST_F(TaskExecutorTest, ExecuteTaskWithInvalidMovePayload) {
    ...
    task.type = TaskType::REPLICA_MOVE;
    task.status = TaskStatus::PENDING;
    task.payload = R"({"key": test_key})"; 

    executor.SubmitTask(task, client_id);

    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    SUCCEED() << "Executor handled invalid JSON payload for move task gracefully.";
...
TEST_F(TaskExecutorTest, TaskRetryCountBoundaryConditions) {
    ...
    // Verify that retry_count can exceed MAX_RETRY_COUNT (the check is in ExecuteTask, not here)
    task.increment_retry();
