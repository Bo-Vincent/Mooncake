...
protected:
    ...
    void SetUp() override {
        ...
        auto client1_opt = Client::Create(
            ...
            FLAGS_protocol,
            FLAGS_device_name.empty() ? std::nullopt : std::make_optional(FLAGS_device_name),
            master_address_
        );
        ...
        auto mount1_result = client1_->MountSegment(client1_segment_ptr_, segment_size);
        ASSERT_TRUE(mount1_result.has_value()) << "Failed to mount segment for client1: "
        ...
        auto mount2_result = client2_->MountSegment(client2_segment_ptr_, segment_size);
        ASSERT_TRUE(mount2_result.has_value()) << "Failed to mount segment for client2: "
                                                << toString(mount2_result.error());

        // Wait for segments to be registered and clients to ping master
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));

        // Client IDs will be extracted from task assignments when tasks are created
    ...
    // When a task is created, we can query it to see which client it's assigned to
    ...
    bool WaitForTaskCompletion(const UUID& task_id, 
    ...
    // Note: segment name is typically the client's hostname (e.g., "127.0.0.1:18001")
    std::string GetSegmentNameFromQuery(const QueryResult& query_result) {
        ...
        if (replica.is_memory_replica()) {
            ...
            // For memory replicas, the segment name is stored in the buffer descriptor
            // but transport_endpoint_ might not match segment name
            ...
            // For now, use transport_endpoint_ as a fallback, but we should verify
            // it matches the actual segment name used during mount
...
TEST_F(TaskExecutorIntegrationTest, ReplicaCopyCompleteFlow) {
    ...
    std::string test_key = "test_copy_key_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    std::string test_data = "This is test data for replica copy operation. " 
    ...
    ASSERT_TRUE(put_result.has_value()) << "Failed to put data: " 
                                         << toString(put_result.error());

    // Wait a bit for put to complete
    std::this_thread::sleep_for(std::chrono::milliseconds(200));

    // Step 2: Verify that client1's data was put successfully and get actual segment name
    auto client1_query_result = client1_->Query(test_key);
    ASSERT_TRUE(client1_query_result.has_value()) << "Failed to query key for verification";
    ASSERT_FALSE(client1_query_result.value().replicas.empty()) << "No replicas found after Put";
    ...
    source_segment = replica.get_memory_descriptor().buffer_descriptor.transport_endpoint_;
    ASSERT_FALSE(source_segment.empty()) << "Failed to extract source segment name";
    
    // Step 3: Determine target segment dynamically (must be different from source)
    // Choose the other client's segment as target to ensure source != target
    ...
    // Verify source and target are different - this test only covers inconsistent cases
    ...
    ASSERT_TRUE(copy_result.has_value()) << "Failed to create copy task: "
                                          << toString(copy_result.error());
    ...
    bool task_completed = WaitForTaskCompletion(task_id, std::chrono::seconds(30));
    ...
    ASSERT_TRUE(client2_query_result.has_value()) << "Failed to query copied key";
    ...
    ASSERT_TRUE(get_result.has_value()) << "Failed to get data from client2: "
                                         << toString(get_result.error());
    
    std::string read_data(reinterpret_cast<const char*>(read_buffer.data()), test_data.size());
...
TEST_F(TaskExecutorIntegrationTest, ReplicaMoveCompleteFlow) {
    // Step 1: Put data on client1
    std::string test_key = "test_move_key_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    std::string test_data = "This is test data for replica move operation. "
    ...
    ASSERT_TRUE(query_result.has_value()) << "Failed to query key for verification";
    ASSERT_FALSE(query_result.value().replicas.empty()) << "No replicas found after Put";
    ...
    auto move_result = master_client_->Move(test_key, source_segment, target_segment);
    ASSERT_TRUE(move_result.has_value()) << "Failed to create move task: "
                                          << toString(move_result.error());
    ...
    ASSERT_TRUE(query_result_client2.has_value()) << "Failed to query moved key from client2";
...
TEST_F(TaskExecutorIntegrationTest, ReplicaCopyToMultipleTargets) {
    // Step 1: Put data on client1
    std::string test_key = "test_multi_target_copy_key_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    std::string test_data = "This is test data for multiple target copy operation.";
    ...
    // Step 3: Determine target segments (both client1 and client2, excluding source)
    ...
    for (const auto& target_segment : target_segments) {
        ...
        ASSERT_TRUE(target_query_result.has_value()) 
            << "Failed to query copied key from target segment: " << target_segment;
...
TEST_F(TaskExecutorIntegrationTest, MultipleCopyTasks) {
    ...
    for (int i = 0; i < num_keys; ++i) {
        std::string key = "test_multi_copy_key_" + std::to_string(i) + "_" 
    ...
    // Step 2: For each key, determine its source segment and create copy task with appropriate target
    // Each key may be on a different segment, so we need to check each one individually
    for (const auto& key : keys) {
        ...
        ASSERT_FALSE(query_result.value().replicas.empty()) << "No replicas found for key: " << key;
        
        // Extract actual source segment name from replica
        std::string source_segment;
        const auto& replica = query_result.value().replicas[0];
        ASSERT_TRUE(replica.is_memory_replica()) << "Expected memory replica for key: " << key;
        source_segment = replica.get_memory_descriptor().buffer_descriptor.transport_endpoint_;
        ASSERT_FALSE(source_segment.empty()) << "Failed to extract source segment name for key: " << key;
        ...
        // For the last key, use multiple target segments to test multi-target copy
        std::vector<std::string> targets;
        size_t key_index = task_ids.size();  // Current index before adding this task
        if (key_index == keys.size() - 1 && keys.size() >= 2) {
            // Last key: use multiple targets if we have at least 2 clients
            if (source_segment == "127.0.0.1:18001") {
                targets = {"127.0.0.1:18002"};  // Only one other segment available
            } else if (source_segment == "127.0.0.1:18002") {
                targets = {"127.0.0.1:18001"};  // Only one other segment available
            } else {
        ...
        ASSERT_TRUE(copy_result.has_value()) << "Failed to create copy task for " << key;
        task_ids.push_back(copy_result.value());
    }

    // Step 4: Wait for all tasks to complete
    for (size_t i = 0; i < task_ids.size(); ++i) {
        bool completed = WaitForTaskCompletion(task_ids[i], std::chrono::seconds(30));
        if (!completed) {
            // Query task to get error message
            auto query_task = master_client_->QueryTask(task_ids[i]);
            if (query_task.has_value()) {
                LOG(ERROR) << "Task " << i << " failed: " << query_task.value().message;
            }
        }
        ASSERT_TRUE(completed) << "Task " << i << " (key: " << keys[i] << ") did not complete";
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        ASSERT_FALSE(query_result.value().replicas.empty()) << "No replicas found for key " << i;
        
        std::string source_segment;
        const auto& replica = query_result.value().replicas[0];
        ASSERT_TRUE(replica.is_memory_replica()) << "Expected memory replica for key " << i;
        source_segment = replica.get_memory_descriptor().buffer_descriptor.transport_endpoint_;
        ...
        if (is_multi_target) {
            ...
            for (const auto& target_seg : target_segments) {
                std::shared_ptr<Client> target_client = (target_seg == "127.0.0.1:18001") 
                                                        ? client1_ : client2_;
                
                auto target_query = target_client->Query(keys[i]);
                ASSERT_TRUE(target_query.has_value()) 
                    << "Failed to query key " << i << " from target segment: " << target_seg;
                ASSERT_FALSE(target_query.value().replicas.empty()) 
                    << "No replicas found for key " << i << " on segment: " << target_seg;
                ...
                ASSERT_TRUE(get_result.has_value()) 
                    << "Failed to get key " << i << " from target segment: " << target_seg;
                ...
                ASSERT_EQ(read_data, test_data_list[i]) 
                    << "Data mismatch for key " << i << " on segment: " << target_seg;
            }
        } else {
            ...
            std::string read_data(reinterpret_cast<const char*>(read_buffer.data()), 
                                 test_data_list[i].size());
            ASSERT_EQ(read_data, test_data_list[i]) << "Data mismatch for key " << i;
        }
    }
}
...
TEST_F(TaskExecutorIntegrationTest, MultipleMoveTasks) {
    ...
    // Step 1: Put multiple keys on client1
    for (int i = 0; i < num_keys; ++i) {
        std::string key = "test_multi_move_key_" + std::to_string(i) + "_" 
                         + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    for (const auto& key : keys) {
        ...
        auto move_result = master_client_->Move(key, source_segment, target_segment);
        ASSERT_TRUE(move_result.has_value()) << "Failed to create move task for " << key;
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if (source_segment == "127.0.0.1:18001" || source_segment.empty()) {
            // Check client2 (may have moved from client1 or was originally on client1)
            target_client = client2_;
        } else {
            target_client = client1_;
        }
        ...
        std::string read_data(reinterpret_cast<const char*>(read_buffer.data()), 
                             test_data_list[i].size());
        ASSERT_EQ(read_data, test_data_list[i]) << "Data mismatch for moved key " << i;
    }
}
...
TEST_F(TaskExecutorIntegrationTest, ConcurrentCopyAndMoveOperations) {
    ...
    for (int i = 0; i < num_copy_keys; ++i) {
        std::string key = "test_concurrent_copy_key_" + std::to_string(i) + "_" 
                         + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    for (int i = 0; i < num_move_keys; ++i) {
        std::string key = "test_concurrent_move_key_" + std::to_string(i) + "_" 
                         + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    for (size_t idx = 0; idx < copy_keys.size(); ++idx) {
        ...
        auto query_result = client1_->Query(key);
        ASSERT_TRUE(query_result.has_value()) << "Failed to query copy key: " << key;
    ...
    for (const auto& key : move_keys) {
        auto query_result = client1_->Query(key);
        ASSERT_TRUE(query_result.has_value()) << "Failed to query move key: " << key;
        ASSERT_FALSE(query_result.value().replicas.empty());
    ...
    for (size_t i = 0; i < copy_task_ids.size(); ++i) {
        bool completed = WaitForTaskCompletion(copy_task_ids[i], std::chrono::seconds(30));
        ASSERT_TRUE(completed) << "Copy task " << i << " (key: " << copy_keys[i] << ") did not complete";
    }

    for (size_t i = 0; i < move_task_ids.size(); ++i) {
        bool completed = WaitForTaskCompletion(move_task_ids[i], std::chrono::seconds(30));
        ASSERT_TRUE(completed) << "Move task " << i << " (key: " << move_keys[i] << ") did not complete";
    }
    ...
    for (size_t i = 0; i < copy_keys.size(); ++i) {
        ...
        if (is_multi_target) {
            ...
            for (const auto& target_seg : target_segments) {
                ...
                ASSERT_TRUE(target_query.has_value()) 
                    << "Copy key " << i << " not found on target segment: " << target_seg;
                ASSERT_FALSE(target_query.value().replicas.empty()) 
                    << "No replicas for copy key " << i << " on segment: " << target_seg;
            }
        } else {
            // Verify on single target segment
            std::shared_ptr<Client> target_client = (source_segment == "127.0.0.1:18001") 
                                                    ? client2_ : client1_;

            auto target_query = target_client->Query(copy_keys[i]);
            ASSERT_TRUE(target_query.has_value()) << "Copy key " << i << " not found on target";
            ASSERT_FALSE(target_query.value().replicas.empty()) << "No replicas for copy key " << i;
        }
    }
    ...
    for (size_t i = 0; i < move_keys.size(); ++i) {
        ...
        std::string source_segment;
        if (query_result.has_value() && !query_result.value().replicas.empty()) {
            const auto& replica = query_result.value().replicas[0];
            if (replica.is_memory_replica()) {
                source_segment = replica.get_memory_descriptor().buffer_descriptor.transport_endpoint_;
            }
        ...
        std::shared_ptr<Client> target_client = (source_segment == "127.0.0.1:18001" || source_segment.empty()) 
                                                ? client2_ : client1_;

        auto target_query = target_client->Query(move_keys[i]);
        ASSERT_TRUE(target_query.has_value()) << "Move key " << i << " not found on target";
        ASSERT_FALSE(target_query.value().replicas.empty()) << "No replicas for move key " << i;
    }
}
