...
tl::expected<UUID, ErrorCode> Client::Copy(const std::string& key, 
                ...
                if (fetch_result.has_value()) {
                    ...
                    if (!tasks.empty()) {
                        ...
                        for (const auto& task_assignment : tasks) {
                            // Convert TaskAssignment to Task
                            Task task;
                            task.id = task_assignment.id;
                            task.type = task_assignment.type;
                            task.status = TaskStatus::PROCESSING;  // Already set by master
                            task.payload = task_assignment.payload;
                            // Convert epoch milliseconds to system_clock::time_point
                            task.created_at = std::chrono::system_clock::time_point(
                                std::chrono::duration_cast<std::chrono::system_clock::duration>(
                                    std::chrono::milliseconds(task_assignment.created_at_ms_epoch)));
                            task.last_updated_at = std::chrono::system_clock::now();
                            task.message = "";
                            task.assigned_client = client_id_;

                            task_executor_->SubmitTask(task, client_id_);
                        }
                    }
                } else {
