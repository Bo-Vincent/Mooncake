...
bool StandbyMetadataStore::RestoreWeightCatalog(
    ...
    for (const auto& record : normalized.metadata) {
        metadata.emplace(record.identity, record);
    }
    for (const auto& lease : normalized.leases) {
        leases.emplace(lease.lease_id, lease);
    }
    for (const auto& operation : normalized.operations) {
    ...
    next_weight_lease_id_ = normalized.next_lease_id;
    next_weight_operation_id_ = normalized.next_operation_id;
    return true;
}

WeightCatalogSnapshot StandbyMetadataStore::SnapshotWeightCatalog() const {
    std::lock_guard<std::mutex> lock(mutex_);
