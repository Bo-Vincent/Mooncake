...
```text
mooncake-model-weight/benchmarks/heterogeneous_weight_reshard/
  case_spec.py
  m2n_adapter.py
  mooncake_adapter.py
  __main__.py
...
source 进程在 source host 完成同样的本地 allocation/registration，使用收到的
target runtime manifest 与本地 source runtime manifest 调用
`plan_runtime_transfer`。实际发送仍由 `MooncakeTransferEngineSink.execute` 完成，
