...
均在 bind 阶段注入。公开接口不提供 one-shot combined runtime manifest；Store
持久化和 TE 执行分别消费同一份逻辑计划及其所需 binding。
...
- 不提供 combined runtime manifest 或兼容投影函数；
- placement identity 必须稳定且由逻辑内容导出，runtime binding 只证明当前实例。
...
- Store source planner 继续使用持久化 `WeightManifest`，runtime source 使用独立
  placement。
...
- `bind_logical_transfer_plan` 接收 target bindings，并通过 `source_bindings` 接收
  source bindings；
- binding 输入只接受 `WeightRuntimeBindingManifest`；
- 双侧均严格校验 placement identity、fragment set、geometry、rank、aliases 和
  nbytes；
- source 跨 DP generation 不一致时 fail closed；
- 绑定后生成 planner 私有的 source/target `BoundWeightFragment`、executor lease snapshots 和
  pipeline routes；
- Store source 不要求 source binding。

### 4. Placement planning and binding

- runtime source 必须直接导出 placement，不从地址对象反向投影；
- logical plan 不含 source 地址；
- 执行时调用方显式把 source runtime bindings 传给 `source_bindings`；
- 最终 `TransferPlan` 在 binding 后生成 operation、executor snapshot 和 route。
...
- placement JSON/digest 与 canonical `shard_dims/parallel_axes`；
...
- Store source、TE reader/sink、canonical 单维 TP 回归；
...
python3.12 -m pytest -q mooncake-reshard/tests/test_weight*.py
```
