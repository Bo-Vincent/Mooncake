...
`master_service.cpp` 中。实现按状态权威、决策和副作用编排拆分为以下 translation
unit：
...
- `master_service_weight_management.cpp` 负责 import、query、policy update、lease、
  operation 创建以及 durable publication；
- `master_service_weight_lifecycle.cpp` 负责 reconciliation、物理 residency 变化、
  managed-group protection 和 deletion；
- `master_service.cpp` 保留普通 Store 主流程、通用 group/eviction 原语，以及调用
  Weight guard/reconciliation 的少量集成点。

这个拆分只移动 `MasterService` 的 out-of-line method definition，不新增第二个
service、repository 或 metadata authority，也不改变公开 API、锁顺序、serialization
和运行语义。`master_service.h` 继续声明统一的 `MasterService` 接口和私有状态。
