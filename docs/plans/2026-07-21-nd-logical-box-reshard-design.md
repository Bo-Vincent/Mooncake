...
- framework adapter 生成的 canonical placement 与 live binding 分别是逻辑语义
  和物理位置的唯一事实来源；
...
- 既有 canonical 单维 TP、Store source 和现有 TE 路径行为保持不变；
...
`TensorDescriptor` 使用唯一的 logical sharding 表示：
...
- `SplitAxis(kind, dim)` 表示 ranks 共同覆盖一个 logical tensor；
- `ReplicatedAxis(kind)` 表示每个 rank 都持有独立完整副本；
- `OwnershipAxis(kind)` 表示只有 manifest 声明的 owner 持有 tensor；
- `SplitAxis` 的维度集合必须与 `shard_dims` 精确相同；
- Mooncake core 不接受 `partition_dim`、可空 `split_dim` 或 duck-typed axis。
...
`PlacementFragment` 和 `StoredFragment` 已经具备 N-D box 所需字段；runtime
执行时再将 placement 绑定为 `BoundWeightFragment`：
...
Grouped expert tensor 可以声明为：

```text
global_offset = [expert_offset, out_offset, in_offset]
local_shape   = [local_experts, local_out, local_in]
shard_dims    = [0, 1] 或 [0, 2]
parallel_axes = [SplitAxis("ep", 0), SplitAxis("tp", 1 或 2)]
```

Mooncake 不解析 `expert`、`gate_proj`、`down_proj` 等名字。把多个 runtime
参数映射到同一个 logical tensor、确定 leading expert coordinate，是 SGLang
等框架 adapter 的职责。独立分配且以 `expert_id` 标识的 tensor 不再把 EP
伪装成 dim0 split，而是使用 `OwnershipAxis("ep")`。

### 4.4 单一契约

- SGLang/vLLM 等 framework adapter 把 runtime inventory 显式规范化为 typed
  canonical values；
- `WeightPlacementManifest` 是完整 global placement，
  `WeightRuntimeBindingManifest` 是单 participant 的 live physical binding；
- `WeightManifest` 始终持久化 canonical `shard_dims` 和 axis semantics；
- Store manifest 是 runtime manifest 的持久化投影，不补充、猜测或改写模型语义；
- 当前接口不提供旧 schema 兼容分支。
...
class TransferRegion:
    tensor_id: str
    source: PlacementFragment | StoredFragment
...
- source 和 target placement 都已绑定为 `BoundWeightFragment`；
...
9. placement + binding -> Store -> placement + binding round-trip；
