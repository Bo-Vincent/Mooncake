...
- `RuntimeManifest` 保存输入 inventory 的 `format_version`；
- v1 parser 和 v1 exact-field JSON schema 保持不变；
- v2 tensor JSON 增加 `shard_dims`；
- `WeightManifest` v1 继续可读写，v2 才持久化 `shard_dims`；
...
PP、EP、TP rank 只是 fragment ownership，覆盖由所有 boxes 的 union 决定。
