...
**Files:**
- Modify: `mooncake-wheel/mooncake/weight_transfer/manifest.py`
- Test: `mooncake-wheel/tests/test_weight_transfer_manifest.py`
...
```python
def test_runtime_manifest_v2_accepts_multi_axis_logical_box():
    ...
    RuntimeManifest(..., tensors=(tensor,), fragments=(fragment,), format_version=2)
```

同时测试：

- v1 `partition_dim` 行为不变；
- v2 多维 shard box 合法；
- 非 shard dimension 上的 partial box 被拒绝；
- `partition_dim` 与 `shard_dims` 冲突被拒绝；
- v1/v2 WeightManifest JSON exact fields 和 round-trip；
- unsupported format version 被拒绝。

**Step 2: 运行 RED**

```bash
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_transfer_manifest.py -q
```

预期：因为 `shard_dims`/runtime format v2 尚不存在而失败。
...
让 `_validate_fragment_geometry` 使用 `effective_shard_dims`。为
`RuntimeManifest` 保存 `format_version`，让 runtime inventory 和 persisted
WeightManifest 分版本解析/序列化。
...
PYTHONPATH=mooncake-wheel python -m pytest \
  ...
  mooncake-wheel/tests/test_weight_store.py -q
```
...
```bash
git add mooncake-wheel/mooncake/weight_transfer/manifest.py \
  mooncake-wheel/tests/test_weight_transfer_manifest.py
git commit -m "feat(weight_transfer): add manifest v2 shard boxes"
```

### Task 2: Backend-neutral TransferRegion

**Files:**
- Modify: `mooncake-wheel/mooncake/weight_transfer/planner.py`
- Modify: `mooncake-wheel/mooncake/weight_transfer/__init__.py`
- Test: `mooncake-wheel/tests/test_weight_transfer_planner.py`
...
```bash
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_transfer_planner.py \
...
```bash
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_transfer_planner.py -q
```

**Step 5: 提交**

```bash
git add mooncake-wheel/mooncake/weight_transfer/planner.py \
  mooncake-wheel/mooncake/weight_transfer/__init__.py \
  mooncake-wheel/tests/test_weight_transfer_planner.py
git commit -m "feat(weight_transfer): define N-D transfer regions"
```
...
git add mooncake-wheel/mooncake/weight_transfer/planner.py \
  mooncake-wheel/tests/test_weight_transfer_planner.py
git commit -m "feat(weight_transfer): plan N-D logical-box overlaps"
```
...
- Modify: `mooncake-wheel/mooncake/weight_transfer/planner.py`
- Test: `mooncake-wheel/tests/test_weight_transfer_planner.py`
- Test: `mooncake-wheel/tests/test_weight_all_axes_native_e2e.py`
...
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_transfer_planner.py \
  mooncake-wheel/tests/test_weight_all_axes_native_e2e.py -q
```
...
git add mooncake-wheel/mooncake/weight_transfer/planner.py \
  mooncake-wheel/tests/test_weight_transfer_planner.py \
  mooncake-wheel/tests/test_weight_all_axes_native_e2e.py
git commit -m "feat(weight_transfer): route PP and EP logical boxes"
```
...
git add mooncake-wheel/mooncake/weight_transfer/planner.py \
  mooncake-wheel/tests/test_weight_transfer_planner.py
git commit -m "fix(weight_transfer): validate N-D physical writes"
```

### Task 6: TE bounded lowering

**Files:**
- Modify: `mooncake-wheel/mooncake/weight_transfer/te.py`
- Test: `mooncake-wheel/tests/test_weight_transfer_te.py`
...
```bash
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_transfer_te.py -q
```
...
```bash
git add mooncake-wheel/mooncake/weight_transfer/te.py \
  mooncake-wheel/tests/test_weight_transfer_te.py
git commit -m "feat(weight_transfer): lower N-D regions in bounded batches"
```

### Task 7: Store manifest v2 和 Store source

**Files:**
- Modify: `mooncake-wheel/mooncake/weight_transfer/store.py`
- Modify: `mooncake-wheel/mooncake/weight_transfer/manifest.py`
- Test: `mooncake-wheel/tests/test_weight_store.py`
- Test: `mooncake-wheel/tests/test_weight_store_gpu_e2e.py`
...
```bash
PYTHONPATH=mooncake-wheel python -m pytest \
  mooncake-wheel/tests/test_weight_store.py \
  mooncake-wheel/tests/test_weight_store_gpu_e2e.py -q
```

**Step 5: 提交**

```bash
git add mooncake-wheel/mooncake/weight_transfer/store.py \
  mooncake-wheel/mooncake/weight_transfer/manifest.py \
  mooncake-wheel/tests/test_weight_store.py \
  mooncake-wheel/tests/test_weight_store_gpu_e2e.py
