...
**Goal:** 基于 canonical manifest + TransferPlan 实现 N-D logical-box reshard
planner，并完成 PP/EP/TP/DP、Store、TE、runtime G2G 和真实推理验证。
...
### Task 1: Canonical N-D manifest schema

**Files:**
- Modify: `mooncake-reshard/python/mooncake/model_weight/manifest.py`
- Test: `mooncake-reshard/tests/test_model_weight_manifest.py`
...
def test_runtime_manifest_accepts_multi_axis_logical_box():
    tensor = TensorDescriptor(
        ...
        itemsize=2,
        shard_dims=(0, 1),
        parallel_axes=(SplitAxis("ep", 0), SplitAxis("tp", 1)),
        layer_id=0,
    )
    ...
    WeightPlacementManifest(..., tensors=(tensor,), fragments=(fragment,))
```
...
- `SplitAxis.dim` 集合与 `shard_dims` 冲突被拒绝；
- WeightManifest JSON exact fields 和 round-trip。

**Step 2: 运行 RED**

```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_manifest.py -q
```
...
在 `TensorDescriptor` 中使用唯一的 `shard_dims` 和显式 axis semantics：

```python
shard_dims: tuple[int, ...]
parallel_axes: tuple[SplitAxis | ReplicatedAxis | OwnershipAxis, ...]
```

让 `_validate_fragment_geometry` 使用 `shard_dims`。Framework adapter 负责把
runtime inventory 规范化为 typed contract；persisted WeightManifest 使用同一套
canonical axis wire semantics，不在 Mooncake core 兼容别名字段。

**Step 4: 运行 GREEN 和旧回归**

```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_manifest.py \
  mooncake-reshard/tests/test_weight_store.py -q
```
...
```bash
git add mooncake-reshard/python/mooncake/model_weight/manifest.py \
  mooncake-reshard/tests/test_model_weight_manifest.py
...
**Files:**
- Modify: `mooncake-reshard/python/mooncake/model_weight/planner.py`
- Modify: `mooncake-reshard/python/mooncake/model_weight/__init__.py`
- Test: `mooncake-reshard/tests/test_model_weight_planner.py`
...
```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_planner.py \
...
```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_planner.py -q
```

**Step 5: 提交**

```bash
git add mooncake-reshard/python/mooncake/model_weight/planner.py \
  mooncake-reshard/python/mooncake/model_weight/__init__.py \
  mooncake-reshard/tests/test_model_weight_planner.py
...
预期：logical box dimension mismatch 或 coverage 失败。
...
source/target `shard_dims`。删除 1-D `_target_interval/_overlap/_copy_ranges`
...
- Modify: `mooncake-reshard/python/mooncake/model_weight/planner.py`
- Test: `mooncake-reshard/tests/test_model_weight_planner.py`
- Test: `mooncake-reshard/tests/test_weight_all_axes_native_e2e.py`
...
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_planner.py \
  mooncake-reshard/tests/test_weight_all_axes_native_e2e.py -q
```
...
git add mooncake-reshard/python/mooncake/model_weight/planner.py \
  mooncake-reshard/tests/test_model_weight_planner.py \
  mooncake-reshard/tests/test_weight_all_axes_native_e2e.py
...
**Files:**
- Modify: `mooncake-reshard/python/mooncake/model_weight/te.py`
- Test: `mooncake-reshard/tests/test_model_weight_te.py`
...
```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_model_weight_te.py -q
```
...
```bash
git add mooncake-reshard/python/mooncake/model_weight/te.py \
  mooncake-reshard/tests/test_model_weight_te.py
...
**Files:**
- Modify: `mooncake-reshard/python/mooncake/model_weight/store.py`
- Modify: `mooncake-reshard/python/mooncake/model_weight/manifest.py`
- Test: `mooncake-reshard/tests/test_weight_store.py`
- Test: `mooncake-reshard/tests/test_weight_store_gpu_e2e.py`
...
```bash
PYTHONPATH=mooncake-wheel:mooncake-reshard/python python -m pytest \
  mooncake-reshard/tests/test_weight_store.py \
  mooncake-reshard/tests/test_weight_store_gpu_e2e.py -q
```

**Step 5: 提交**

```bash
git add mooncake-reshard/python/mooncake/model_weight/store.py \
  mooncake-reshard/python/mooncake/model_weight/manifest.py \
  mooncake-reshard/tests/test_weight_store.py \
  mooncake-reshard/tests/test_weight_store_gpu_e2e.py
