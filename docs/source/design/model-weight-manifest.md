# Model Weight Manifest Contract
...
The implementation is owned by the top-level `mooncake-model-weight`
module. Its public Python API is `mooncake.model_weight`.
...
Manifest validation is local to each `(tensor_id, ParallelRank)` owner. It
rejects overlapping boxes owned by the same rank, but intentionally permits
overlap across different ranks and does not require the fragments to cover the
full `global_shape`. Cross-rank conflicts and complete target-tensor coverage
depend on the requested source and target topologies and are planner-owned
invariants.
...
offset, and shape. Framework exporters that supply explicit
`placement_fragment_id` values must keep them stable across process restarts;
alternating between generated and explicit IDs changes placement identity.
...
contiguous view. Address zero is reserved as a null sentinel and is therefore
invalid for a transferable view. Its half-open address range must have a
representable unsigned 64-bit exclusive end.
...
import SGLang, vLLM, PyTorch, or another model framework. Integer-valued
contract fields require Python `int` values and reject `bool`; adapters must
normalize framework-specific scalar types such as `numpy.int64` before
