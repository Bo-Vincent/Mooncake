...
`mooncake-model-weight` 模块维护。通用传输执行和物理存储管理仍分别属于
...
`mooncake.model_weight` 并提供 Manifest 时才进入该模块；KVCache 仍使用
...
- SGLang worker RPC：远程获取/续租/释放 Runtime Manifest；POC 已实现。
...
- Mooncake 定向模块套件：`346 passed, 20 skipped, 8 subtests passed`。
