...
`-DMOONCAKE_ENABLE_ADAPTIVE_CC=ON` to the CMake configuration. Include
...
Set `MC_ADAPTIVE_CC_MODE` before starting the application:
...
```bash
export MC_ADAPTIVE_CC_MODE=observe
...
| --- | --- | --- |
| `MC_ADAPTIVE_CC_MIN_WINDOW_BYTES` | `4194304` (4 MiB) | Lower bound for the normal adaptive window. |
| `MC_ADAPTIVE_CC_MAX_WINDOW_BYTES` | `67108864` (64 MiB) | Upper bound for the normal adaptive window. |
| `MC_ADAPTIVE_CC_TARGET_DRAIN_US` | `2000` | Target estimated queue-drain time, in microseconds. |
| `MC_ADAPTIVE_CC_HIGH_PRESSURE_EPOCHS` | `2` | Consecutive high-pressure epochs before shrinking the window. |
| `MC_ADAPTIVE_CC_LOW_PRESSURE_EPOCHS` | `3` | Consecutive low-pressure epochs before growing the window. |
| `MC_ADAPTIVE_CC_HARD_ERROR_THRESHOLD` | `3` | Hard errors before quarantining the path. |
| `MC_ADAPTIVE_CC_COOLDOWN_MS` | `30000` | Quarantine cooldown, in milliseconds. |
| `MC_ADAPTIVE_CC_PROBE_WINDOW_BYTES` | `65536` | Recovery probe window, in bytes. |
...
See the [Chinese startup-parameter reference](adaptive-cc-startup-config.md)
...
transfers, and restart with `MC_ADAPTIVE_CC_MODE=off`. There is no live mode-switch
API. Rebuilding with `MOONCAKE_ENABLE_ADAPTIVE_CC=OFF` also removes the controller
